...
class SNModelScipyDR(object):
    ...
    def fit(self, times, fluxpsf, obs_errors):
        ...
        trise_bounds = [1.0, 100.0]
        tfall_bounds = [1.0, 180.0]
        C_bounds = [np.amin(fluxpsf), max_fluxpsf]
        ...
        trise_guess = 3.0
        tfall_guess = 50.0
        C_guess = np.percentile(fluxpsf, 5)

        # reference guess
        p0 = [A_guess, t0_guess, gamma_guess,
              beta_guess, trise_guess, tfall_guess,C_guess]
        ...
        try:
            pout, pcov = curve_fit(
                ...
                p0=p0,
                sigma=5+obs_errors,
                bounds=[[A_bounds[0], t0_bounds[0], gamma_bounds[0], beta_bounds[0], trise_bounds[0], tfall_bounds[0], C_bounds[0]],
                        [A_bounds[1], t0_bounds[1], gamma_bounds[1], beta_bounds[1], trise_bounds[1], tfall_bounds[1], C_bounds[1]]],
                ftol=1e-8,
                # verbose=2
            )
        except (ValueError, RuntimeError, OptimizeWarning):
            try:
                ...
                logging.info('Two fits of SPM failed. Returning NaN.')
                pout = np.array([np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan])
...
class SPMExtractorDR(FeatureExtractorSingleBand):
    """Fits a SNe parametric model to the light curve and provides
    the fitted parameters as features."""
