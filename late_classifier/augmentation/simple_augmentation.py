...
class Augmenter:
    def augment(self, detections: pd.DataFrame, non_detections: pd.DataFrame, labels: pd.DataFrame, multiplier: int):
        raise NotImplementedError('Augmenter is an abstract class')

    def _get_augmentable_oids(self, labels: pd.DataFrame, augmentable_classes):
        useful_rows = labels[labels.classALeRCE.isin(augmentable_classes)]
...
class ShortTransientAugmenter(Augmenter):
    ...
    def augment(
        ...
        for template_oid in augmentable_oids:
            ...
            new_lengths = np.random.choice(possible_lengths, min(len(possible_lengths), multiplier), replace=False)
        ...
        new_detections.append(detections.copy())
        new_non_detections.append(non_detections.copy())
        new_labels.append(labels.copy())
