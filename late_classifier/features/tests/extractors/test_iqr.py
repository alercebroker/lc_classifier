...
class TestIQRExtractor(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_many_objects(self):
        iqr_extractor = IQRExtractor()
        iqr = iqr_extractor.compute_features(self.detections)
        self.assertEqual(iqr.shape, (4, 2))
        nan_values = iqr.isna().values
        expected_nans = [
            [False, False],
            [True, False],
            [True, False],
        ...
        self.assertTrue((nan_values == expected_nans).all())
